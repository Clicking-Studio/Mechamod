...
const snake = loader.load('../src/assets/Snake_KeyCap_VI_STL_001.stl', (geometry) => {
    ...
    const shenon = loader.load('../src/assets/shenron-keycap-v1.stl', (geometry) => {
        const material = new THREE.MeshNormalMaterial();
        objects.push(new THREE.Mesh(geometry, material))
        scene.add(objects.at(-1))
        objects.at(-1).position.setZ(hiddenZoneZ)

        const model3 = loader.load('../src/assets/model3.stl', (geometry) => {
            const material = new THREE.MeshNormalMaterial();
            objects.push(new THREE.Mesh(geometry, material))
            scene.add(objects.at(-1))
            objects.at(-1).position.setZ(hiddenZoneZ)

            const model4 = loader.load('../src/assets/model4.stl', (geometry) => {
                const material = new THREE.MeshNormalMaterial();
                objects.push(new THREE.Mesh(geometry, material))
                scene.add(objects.at(-1))
                objects.at(-1).position.setZ(hiddenZoneZ)
